...
async function initializeJacoPerformance(){
  ...
  // Load Intervals.icu before calculating coach decisions.
  await loadWellnessDashboard();
...
window.addEventListener("error",event=>{
  console.error("Jaco Performance error:",event.error || event.message);
  const status=document.getElementById("todayStatus");
  if(status && !status.textContent){
    status.className="status error";
    status.textContent="Een onderdeel kon niet worden geladen. Vernieuw de pagina.";
  }
});

window.addEventListener("unhandledrejection",event=>{
  console.error("Jaco Performance promise error:",event.reason);
});
